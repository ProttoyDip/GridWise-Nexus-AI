...
import { Activity, ArrowDownRight, ArrowUpRight, BatteryCharging, CheckCircle2, CircleDashed, CloudSun, Gauge, Zap } from "lucide-react";
import { Bar, BarChart, CartesianGrid, Cell, ComposedChart, Line, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts";
import type { DirectiveInterpretation, HourlyPlan, OptimizeResponse } from "@/types";

type ResultsPanelProps = { result: OptimizeResponse | null; loading: boolean };
...
function DirectiveCard({ item, index }: { item: DirectiveInterpretation; index: number }) {
  return <motion.article className="directive-card" initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.06, duration: 0.28 }}>
    ...
    <div className="directive-title-row"><h4>{item.directive_type || "Unclassified directive"}</h4><Activity size={15} /></div>
    <p>{item.explanation || "No explanation returned for this directive."}</p>
    <pre>{item.structured_adjustment === null ? "null" : JSON.stringify(item.structured_adjustment, null, 2)}</pre>
  </motion.article>;
}
...
function PlanTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ dataKey?: string; value?: number; color?: string }>; label?: string }) {
  ...
  return <div className="chart-tooltip"><div className="tooltip-label">{String(label).padStart(2, "0")}:00 dispatch</div>{payload.filter((entry) => entry.dataKey !== "actionIndex").map((entry) => <div className="tooltip-row" key={entry.dataKey}><span style={{ background: entry.color }} />{entry.dataKey === "grid_kwh" ? "Grid" : "Solar used"}<strong>{Number(entry.value).toFixed(1)} kWh</strong></div>)}</div>;
}
